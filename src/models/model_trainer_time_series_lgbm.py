...
class LGBMGroupTrainer:
    ...
    def _generate_future_forecast(
        ...
        """Generate future forecasts using iterative prediction."""
        hist_vals = series_g.values.astype(float)
        last_window = hist_vals[-self.n_lags:].copy()
        last_exog = g_feat.iloc[-1] if not g_feat.empty else None
        ...
        for _ in range(forecast_horizon):
            ...
            # Ensure categorical features use same categories
            for c in cat_cols:
